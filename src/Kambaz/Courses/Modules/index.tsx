...
export default function Modules() {
  ...
  useEffect(() => {
    if (!cid) return;
    findModulesForCourse(cid).then((mods) => dispatch(setModules(mods as Module[])));
  }, [cid, dispatch]);
  const handleAddModule = async () => {
    if (!cid || !moduleName.trim()) return;
    const newMod = await createModuleForCourse(cid, { name: moduleName });
    dispatch(addModule(newMod as Omit<Module, "_id" | "editing" | "lessons">));
    setModuleName("");
  };
  ...
  const handleSaveModule = async (mod: any) => {
    ...
    dispatch(updateModuleAction(updated as Module));
  };
